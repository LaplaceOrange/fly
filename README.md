...
3. 针对接收者的每台已注册设备执行 X25519，使用 HKDF-SHA-256 派生独立包装密钥。
4. 内容使用 AES-256-GCM 加密，内容密钥再由每台设备的包装密钥通过 AES-256-GCM 加密。
5. Ed25519 签名覆盖密文、接收者 ID、临时公钥和全部密钥信封。
6. 服务器只保存密文、签名、临时公钥和包装后的密钥；只有指定接收者持有对应私钥的设备能够解密。
