...
- 指定接收者端到端加密：X25519 密钥交换、HKDF-SHA-256 密钥派生、AES-256-GCM 内容加密。
...
每个浏览器设备在登录后生成两组独立设备密钥：

- Ed25519 用于签名和验证分享内容。
- X25519 用于与发送方的临时 X25519 密钥完成密钥交换。
- 每个 X25519 公钥注册前必须由同一设备的 Ed25519 私钥签名，绑定签名同时覆盖 CPOAuth 用户 ID、Ed25519 密钥 ID、算法和 X25519 公钥。
- 两个私钥均以不可导出的 `CryptoKey` 保存在该浏览器 IndexedDB 中；服务器只保存公钥和指纹。
- 每份分享在浏览器签名，服务器写入前验证一次，接收方解密前再次验证。

启用端到端加密时：

1. 分享者必须选择一名已经注册 X25519 设备公钥的接收用户。
2. 浏览器生成随机 AES-256 内容密钥和临时 X25519 密钥对。
3. 发送端先使用接收设备的 Ed25519 公钥验证 X25519 公钥绑定签名，任何未签名或绑定不一致的密钥都会被拒绝。
4. 针对接收者的每台已验证设备执行 X25519，使用 HKDF-SHA-256 派生独立包装密钥。
5. 内容使用 AES-256-GCM 加密，内容密钥再由每台设备的包装密钥通过 AES-256-GCM 加密。
6. Ed25519 分享签名覆盖密文、接收者 ID、临时公钥和全部密钥信封。
7. 服务器只保存密文、签名、临时公钥和包装后的密钥；只有指定接收者持有对应私钥的设备能够解密。

发送界面会列出本次参与密钥交换的接收者设备公钥指纹，便于双方通过独立渠道核对。公钥目录由本站按 CPOAuth 用户身份绑定；若威胁模型包含服务器主动替换公钥，应在发送前完成指纹核验。

新版链接不在 URL 中携带 AES 密钥。历史 `#key=` 分享仍可打开，但不会再创建新的旧版链接。现代加密分享要求 HTTPS 或 localhost，并要求浏览器 Web Crypto 支持 Ed25519 与 X25519。

从未绑定版本升级时，数据库中的旧 X25519 目录条目不会继续用于创建新分享；用户下次登录后，浏览器会自动用现有 Ed25519 私钥签署并重新注册同一 X25519 公钥。已有分享及其设备私钥不受影响。
